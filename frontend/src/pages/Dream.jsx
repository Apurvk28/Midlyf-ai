...
const Dream = () => {
  ...
  const handlePredict = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5001/api/features/dream", {
        method: "POST",
      ...
      setResult("The stars are currently clouded. Try again later.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full h-full overflow-y-auto px-6 py-12 flex justify-center scrollbar-hide">
      <div className="w-full max-w-3xl flex flex-col items-center">
        ...
        <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-white mb-6 text-center leading-tight">
          Decode Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-500">Subconscious</span>.
        </h2>
        <p className="text-gray-400 text-lg text-center max-w-xl mb-12 leading-relaxed">
          Describe your dream. Our AI will analyze its hidden psychological meaning, future implications, and connections to your waking reality.
        </p>
        ...
        <div className="w-full bg-[#111111]/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 shadow-2xl">
          <form onSubmit={handlePredict} className="flex flex-col gap-6">
            <div className="space-y-4">
              <label className="text-sm font-semibold text-gray-400 pl-2">Your Dream</label>
              <textarea
                ...
                placeholder="e.g. I was flying over a futuristic city covered in neon lights, but suddenly I started falling..."
                value={input}
            ...
            >
              {loading ? "Analyzing Dream..." : "Reveal Hidden Meaning"}
            </button>
          </form>
        </div>

        {result && (
          <div className="w-full mt-10 bg-[#111111]/90 backdrop-blur-3xl border border-purple-500/20 p-10 rounded-[2rem] shadow-2xl animate-fade-in relative overflow-hidden">
             {/* Purple ambient glow inside card */}
             <div className="absolute top-[-50px] right-[-50px] w-[200px] h-[200px] bg-purple-600/20 blur-[80px] rounded-full pointer-events-none"></div>

             <h3 className="text-purple-400 font-bold uppercase tracking-widest text-xs mb-6 relative z-10">Dream Reading</h3>
